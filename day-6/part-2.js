...
const main = async () => {
  ...
  let population = 0;
  let fishCountByTimerTick = [0, 0, 0, 0, 0, 0, 0, 0, 0];     // stores how much fish there is in each time tick, from 0 (the ones who will create a new generation) to 9 (the newborns). The newborns start at 9 because they don't decrease the tick at first, only after the second day of life.

  initialState.forEach(tick => fishCountByTimerTick[tick]++); // initialize the counters with the tickers received as input

  for (let i = 0; i < 256; i++) {
    fishCountByTimerTick[7] += fishCountByTimerTick[0];       // everyone that hits 0 will restart with the timer at 6. Here, they go to 7 because we still need to decrease the tickers, so they can go with the ones currently at 7.
