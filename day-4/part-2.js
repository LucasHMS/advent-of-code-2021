...
const main = async () => {
  ...
  for (let drawnNumber of numberSet) {
    ...
    for (let board of boardSet) {
      ...
      if (!winningBoardIdxs.includes(boardIdx) && boardWon(boardMarksSet[boardIdx])) {
        winningBoardIdxs.push(boardIdx);
        lastWinningScore = calculateScore(board, boardMarksSet[boardIdx], drawnNumber);
      }
