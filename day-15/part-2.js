...
const main = async () => {
  ...
  console.time('input parse and expansion');
  caveMap = await parseIinput();
  console.timeEnd('input parse and expansion');
...
const parseIinput = async () => {
  ...
  return expandMap(caveMap);
};

const expandMap = (caveMap) => {
  // horizontal expansion
  caveMap.forEach((line, index) => {
    let newLine = [].concat(line);

    for (let i = 0; i < 4; i++) {
      let newBlock = line.map(r => r + 1 <= 9 ? r + 1 : 1);
      newLine = newLine.concat(newBlock)
      line = [].concat(newBlock);
    }

    caveMap[index] = newLine;
  });

  // vertical expansion
  const lengthBeforeVerticalExpansion = caveMap.length;
  for (let j = 0; j < 4; j++) {
    let newLines = caveMap.slice(j * lengthBeforeVerticalExpansion);

    newLines.forEach(line => {
      let newLine = line.map(r => r + 1 <= 9 ? r + 1 : 1);
      caveMap.push(newLine);
    });
  }
